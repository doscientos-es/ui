...
export function AccordionTrigger({ className, children, ...props }: React.ComponentProps<typeof Button>) {
  return <Button data-slot="accordion-trigger" className={cn("group/accordion-trigger flex w-full items-center justify-between py-3 text-left text-sm font-medium outline-none transition-colors hover:text-primary focus-visible:ring-3 focus-visible:ring-ring/50", className)} {...props}>{(values) => <><span>{typeof children === "function" ? children(values) : children}</span><CaretDownIcon aria-hidden="true" className="size-4 transition-transform group-data-expanded/accordion-trigger:rotate-180 motion-reduce:transition-none" /></>}</Button>;
}
