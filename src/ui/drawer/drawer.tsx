...
const sideStyles = {
  left: 'inset-y-0 left-0 h-full w-[min(22rem,calc(100%-2rem))] data-entering:animate-ui-surface-in',
  right:
    'inset-y-0 right-0 h-full w-[min(22rem,calc(100%-2rem))] data-entering:animate-ui-surface-in',
  bottom: 'inset-x-0 bottom-0 max-h-[85vh] w-full data-entering:animate-ui-surface-in',
}
...
export function Drawer({ children, side = 'right', className, ...props }: DrawerProps) {
  return (
    <ModalOverlay
      isDismissable
      className="motion-safe:data-entering:animate-ui-overlay-in motion-safe:data-exiting:animate-ui-overlay-out fixed inset-0 z-50 bg-black/20 backdrop-blur-[1px]"
      {...props}
    >
      <Modal
        className={cn(
          'absolute grid gap-4 overflow-y-auto rounded-xl border border-border bg-background p-5 text-foreground shadow-xl outline-none',
          sideStyles[side],
          className,
        )}
      >
        <AriaDialog data-slot="drawer" className="outline-none">
          {children}
        </AriaDialog>
      </Modal>
    </ModalOverlay>
