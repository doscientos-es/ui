...
export const Default: Story = {
  ...
  play: async ({ canvasElement }) => { const canvas = within(canvasElement); const page = within(canvasElement.ownerDocument.body); await userEvent.click(canvas.getByRole("button", { name: "Ver detalles" })); await waitFor(() => expect(page.getByText("Cliente activo")).toBeVisible()); await userEvent.keyboard("{Escape}"); await waitFor(() => expect(page.queryByText("Cliente activo")).not.toBeInTheDocument()); },
};
