...
export function Popover({ className, ...props }: PopoverProps) {
  return (
    <AriaPopover
      ...
      offset={6}
      className={cn(
        'min-w-48 rounded-xl border border-border bg-background p-1.5 text-foreground shadow-lg outline-none motion-safe:data-entering:animate-ui-surface-in motion-safe:data-exiting:animate-ui-surface-out',
        className,
      )}
      {...props}
    />
