...
export const Default: Story = {
  ...
  render: () => { const [page, setPage] = useState(2); return <Pagination page={page} pageCount={8} onPageChange={setPage} />; },
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    await userEvent.click(canvas.getByRole("button", { name: "Página 3" }));
    await expect(canvas.getByRole("button", { name: "Página 3" })).toHaveAttribute("aria-current", "page");
